...
//  Blocstagram
//
//  Created by Inioluwa Work Account on 07/06/2016.
...
- (UICollectionViewCell*) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.collectionView = self.filterCollectionView;

    [cell setThumbnailImage:self.filterImages[indexPath.row]];
    [cell setLabelText:self.filterTitles[indexPath.row]];
...
- (void) alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        if (caption.length > 0) {
            _documentController.annotation = @{@"InstagramCaption": caption};
        }
        
        if (self.sendButton.superview) {
            [_documentController presentOpenInMenuFromRect:self.sendButton.bounds inView:self.sendButton animated:YES];
        } else {
            [_documentController presentOpenInMenuFromBarButtonItem:self.sendBarButton animated:YES];
        }
    }
}
